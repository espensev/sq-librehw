...
| plan-003 | Move the Avalonia Fixture Explorer into experiments/ | executed | implemented | 7 | 2 | 0 | `docs/campaign-plan-003-move-the-avalonia-fixture.md` |
...
| 7 | A clean candidate built from the moved source contains zero Avalonia or spike entries in both WinForms package inventories. It is not promoted. | open | Candidate not yet built: `New-LhmRelease.ps1` requires a committed clean tree for a promotable candidate and refused the dirty working tree. The candidate is built from the committed tree immediately after this campaign commit and this row is updated in the follow-up evidence. |
