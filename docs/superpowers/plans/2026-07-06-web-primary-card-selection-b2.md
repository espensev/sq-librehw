...
**Post-verification fix (advisor-caught, commit `6cf79a1`): seeded heroes kept curated labels.** A stronger review pass caught what the id/count/tag verdicts structurally missed: because seed-from-visible (decision 2) pulls the auto heroes into `primaryCards`, the original `resolvePrimaryCards` (`label: s.text`) **relabeled every seeded hero to raw sensor text** the instant custom mode engaged — e.g. the GPU cluster became three ambiguous "GPU CORE" tiles, "CPU Temp" → "Core (Tctl/Tdie)". Labels-only (bounds/units already matched `pickHero`), but it half-undermined decision 2's own non-disruption goal. Fix: `resolvePrimaryCards` builds a `pickHero` id→hero map and returns the curated hero object for seeded heroes, falling back to raw text only for genuine non-hero promotions. Verified live: custom-mode labels now read `["CPU Temp","CPU Load","CPU Power","GPU Temp 1","GPU Mem Jct 1","GPU Load 1","GPU Power 1","GPU Load 2","RAM Used","Drive Temp","Fan #7","Fan #5","Fan #1","Fan #6","CCD1 (Tdie)"]` — heroes curated, the promoted non-hero raw. **Empty-set edge case** also confirmed: removing all primary cards → "0 selected", Auto button stays, no error, Auto recovers the 14 heroes. Final selftest **192/192**.
