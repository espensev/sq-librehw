...
| **D2** | Expansion **anchored-overlay** layout (multi-column detail, zero displacement) | audit finding + operator feedback | Expanded detail fills width, not a tall narrow strip, **and no other card moves**. **Spec Accepted 2026-07-07: [`feature-web-dashboard-expansion-layout.md`](../../feature-web-dashboard-expansion-layout.md)** — §9 #1 re-resolved same day from B grid-breakout to **anchored overlay** after operator feedback ("moves all the cards down, ugly transition"); **keep single `c:` key**. **Plan ready (rev 2): [`2026-07-07-web-expansion-multicolumn-d2.md`](2026-07-07-web-expansion-multicolumn-d2.md)** — 2 tasks (overlay render/position; single-open + click-away + one-shot entrance), controller-owned live displacement gate RED `moved>0` → GREEN `moved=0` + both-theme screenshots. |
| **D2a** | Direct flight-deck edit controls (operator 2026-07-07: "freely add or remove … directly from the card, not a sub menu") | operator feedback | Primary add/remove becomes a first-class control on the visible item: remove-from-primary in the PFD card hover cluster, add-to-primary on pinned cards + panel rows, and a primary toggle in the Sensors popover rows (today rows offer only Pin/Hide, `console.js:1298-1300`; the sole primary toggle is one button among ~12 in `.xp-actions`, `console.js:1055`; the cluster's ⊘ is dashboard-wide hide, not deck removal). Model + delegated actions already exist (`SQ.setPrimaryCard`, `primary-add`/`primary-remove`, `console.js:1405-1406`) — this is presentational surface only. Needs its own mini-plan; watch cluster width on touch (D1 gutter tradeoff). |
...
in parallel with anything; C was independent of B. **D1 done** (card header reserved gutter, merged). **Next: D2** — anchored-overlay expansion (plan rev 2 ready), then **D2a** — direct flight-deck edit controls, then D3.
