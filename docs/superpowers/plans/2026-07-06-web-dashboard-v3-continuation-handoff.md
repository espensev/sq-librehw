...
|---|---|---|
| Responsive/theme fit is not closed | D3 has not yet run the full viewport/theme matrix after D1/D2/D2a | Run `2026-07-07-web-responsive-theme-qa-d3.md`; fix or explicitly accept every patch-list item |
| Sensors popover narrow-width behavior is unproven | D2a added an extra visible-row action; `.sensors-panel` did not get the same mobile anchoring as the Pages menu | Prove viewport fit and readable row actions at 320/390 in dark/light; fix with mobile anchoring or stacked actions if needed |
| Row and panel controls need touch/narrow proof | Row controls remain absolute overlays; a 2026-07-07 live 390-touch pass found `rowOverlapCount=11` and `rowValueOverflowCount=11`; panel headers and adapter headers carry always-visible ▲▼/⊘ controls; `#panelsReset` shares the Subsystems header | Browser rect probes and screenshots must prove no overlap/clipping, and D3 must land scoped CSS/layout fixes for the confirmed row-value overlap |
...
| Visual polish is not complete | Card controls, popovers, overlays, rows, and section headers still need systematic viewport/theme QA | No overlap/clipping at 320/390/640/~768/1440/wide; dark and light both polished |
...
**D2 and D2a are now executed, live-gated, merged, and pushed** (see §0, card-truth §11, and the SDD ledger).
The next work item is **D3 — full responsive/theme QA matrix plus user-perspective polish gate**, now expanded into
[`2026-07-07-web-responsive-theme-qa-d3.md`](2026-07-07-web-responsive-theme-qa-d3.md) and reviewed in
[`../../reviews/review-2026-07-07-dashboard-d3-user-perspective.md`](../../reviews/review-2026-07-07-dashboard-d3-user-perspective.md). Carry §12 forward:
D3 has only limited DOM-less test surface, and a green geometry probe can still hide a visible regression
(D1's Option A passed every overlap check yet truncated names at rest). It also has a confirmed current blocker:
390-touch row controls overlap values. After D3: **E** (viewTheme selector +
`cardtruth` retirement, §5 Slice 7), then **F** (context dashboards — separate campaign, §3.1).

## 11. Progress Log (A → D2a, on `master`)

Reverse chronological; each phase has its own execution record in this folder.

| Phase | What landed | Merge / commit |
|---|---|---|
| **D2a** | Direct flight-deck edit controls (operator finding: primary add/remove was buried in expansion). A ★/☆ star toggle joins the shared card/row control cluster; the Sensors popover gets a matching "Make/Remove primary" button on visible rows; `state.primaryIds` is derived once per render from the same visible sensor set used by `renderPFD`; no new persisted state. Controller live gate fixed two issues before merge: touch/coarse-pointer hides `.ctl.hide` so the fourth button does not reintroduce card-name truncation, and popover primary buttons are limited to `visibility==='visible'` rows. selftest 227/227, golden 42/42, rebuild 0/0, console clean, dark+light live verified; post-review fix `3571311` keeps auto-mode promoted heroes starred truthfully. | `0279333` merge; commits `f310f24`, `f9105c2`, `b491c2b`, `3571311` plus docs |
