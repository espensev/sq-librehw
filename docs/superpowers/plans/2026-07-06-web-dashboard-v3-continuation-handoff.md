...
**D2, D2a, and D3 are now executed and live-gated** (see §0, card-truth §11, and the D3 patch list closeout).
D3 is recorded in [`2026-07-07-web-responsive-theme-qa-d3.md`](2026-07-07-web-responsive-theme-qa-d3.md) and reviewed in
...
(D1's Option A passed every overlap check yet truncated names at rest). The confirmed 390-touch row-control
overlap is fixed by the D3 in-flow touch/narrow row layout. Next: **E** (viewTheme selector +
`cardtruth` retirement, §5 Slice 7), then **F** (context dashboards — separate campaign, §3.1).

## 11. Progress Log (A → D3, on `master`)

Reverse chronological; each phase has its own execution record in this folder.

| Phase | What landed | Merge / commit |
|---|---|---|
| **D3** | Responsive/theme QA matrix plus user-perspective polish. CSS-only product fixes: mobile Sensors post-base anchoring and `62vh` cap, stacked mobile Sensors row actions, touch/narrow `.row-ctl` in-flow below values, mobile section/panel header wrapping, and touch `.ctl` minimum 24 px. Final Edge matrix covered 320x568 touch, 390x844 touch, 640x900 hover+touch, 768x1024 touch, 1440x900 hover, and 1920x1080 hover in dark+light: `maxRowOverlap=0`, `maxRowValueOverflow=0`, no horizontal scroll, Sensors/Pages/overlay fit true, zero console warnings/errors. Stateful probe seeded `panelOrder` + one hidden active NIC; `#panelsReset` and hidden-adapter restore row stayed in viewport at 320/390/640, `panelHeaderBadCount=0`. Accepted baselines: narrow row/source ellipsis and the known ~768 touch `GPU Mem J...` card label truncation; primary value+unit hard-clip count is 0. | D3 follow-up commit on top of `5a95bd7`; docs closeout in `2026-07-07-web-responsive-theme-qa-d3.md` and card-truth §11 |
