...
**Evidence:** operator-annotated screenshot `Screenshot 2026-07-04 082637_edited.png` and desktop sensor tree screenshot `ui-needadj-textpng.png` — both reviewed 2026-07-04; the files were removed from the working tree by a concurrent session before this spec was committed, so the load-bearing annotation is quoted verbatim: the Customize drawer crossed out with "VERY LIMITED USE OF THIS TYPE OF MENU. DO IT ON CARDS". Third source: live `data.json` walk on SND-DESK 2026-07-04 (575 leaf sensors).
...
9. **Branch strategy for UI testing** (operator asked for testable branches): keep model-truth work on `master` (phases A+B — it changes *what* is shown, not the interaction shell; the former `feature/web-dashboard-customization` trunk was merged into master and deleted on 2026-07-04), then two UI branches off master the operator can A/B live: `feat/web-card-first` (drawer removal + expansion + overlap fix) and `feat/web-row-subgroup-order` (row/adapter arranging). Optional `feat/web-limit-sensors` (server) stays parked until explicitly green-lit.
