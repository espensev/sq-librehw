...
|-------|------|-------|------------|------|
| 0 | Start from a clean, immutable `origin/main` checkpoint and isolate utility work on its own branch or worktree. | No product edits | — | Starting SHA is recorded and the utility lane has no inherited dirty files. |
