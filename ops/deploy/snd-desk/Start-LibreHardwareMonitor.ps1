...
function Get-LibreHardwareMonitorProcess {
    ...
    if (-not $InspectionOnly -and $mismatched.Count -gt 0) {
    ...
    if (-not $InspectionOnly -and $entries.Count -gt 1) {
...
if ($ValidateScriptOnly) {
    ...
    # Inspection deliberately reports rather than enforces path ownership so a
    # source-only check cannot be blocked by another machine's live runtime.
    $detectedProcesses = @(Get-LibreHardwareMonitorProcess -InspectionOnly)
    [pscustomobject]@{
        Result = 'PASS'
